...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options.get('all', False) or options.get('structure', False):
            ...
            LOG.log(
                "Automatic import activated: Generating installations", force=True)
        ...
        if repos:
            for d in repos:
                ...
                try:
                    l = Loader(
                        f'https://github.com/DHRI-Curriculum/{repo}', branch, force_download=True)
                except MissingRequiredSection as e:
                    LOG.error(
                        f"One or more required section(s) could not be found in {repo}: {e}")

                if options.get('all', False):
                    repo_name = l.meta['repo_name']
                else:
                    repo_name = get_or_default(
                        'Repository', l.meta['repo_name'])

                repo_name = get_or_default(
                    'Workshop name', auto_replace(repo_name.title()))
                ...
                if l.has_lessons:
                    for lesson_data in l.as_html.lessons:
                        ...
                        if lesson_data['challenge']:
                            ...
                            LOG.created(created, 'Challenge',
                                        challenge.text[:20] + '...', challenge.id)
                        ...
                        if lesson_data['solution']:
                            try:
                                ...
                                LOG.error(
                                    f'Lesson `{lesson.title}` has a solution but no challenge. Will continue but not insert the solution.', kill=False)

                        if lesson_data['evaluation']:
                            eval, created = Evaluation.objects.get_or_create(
                                lesson=lesson
                            )
                            LOG.created(created, 'Evaluation',
                                        eval.lesson.title, eval.id)
                            for d in lesson_data['evaluation']:
                                ...
                                if question:
                                    ...
                                    LOG.created(
                                        created, 'Question', q.label, q.id)

                                    answers = d.get('answers')
                                    if answers:
                                        correct_answers = answers.get(
                                            'correct', [])
                                        incorrect_answers = answers.get(
                                            'incorrect', [])
                                        has_answers = len(correct_answers) > 0 or len(
                                            incorrect_answers) > 0
                                        if has_answers:
                                            for answer in correct_answers:
                                                ...
                                                LOG.created(
                                                    created, 'Answer', obj.label, obj.id)
                                    ...
                                    LOG.warning(
                                        f"Question could not be interpreted in lesson {lesson.title}: Raw data — {d}")
                ...
                for model in l.frontmatter_models:
                    ...
                    if model == Frontmatter:
                        ...
                        LOG.created(created, 'Frontmatter for',
                                    frontmatter.workshop, frontmatter.id)

                    elif model == EthicalConsideration:
                        for label in l.ethical_considerations:
                            obj, created = model.objects.get_or_create(
                                frontmatter=frontmatter, label=label)
                            LOG.created(created, 'Ethical Consideration',
                                        obj.label[:20] + '...', obj.id)
                        ...
                        for label in l.learning_objectives:
                            obj, created = model.objects.get_or_create(
                                frontmatter=frontmatter, label=label)
                            LOG.created(created, 'Learning Objective',
                                        obj.label[:20] + '...', obj.id)
                        ...
                        for annotation in l.projects:
                            ...
                            obj, created = model.objects.get_or_create(
                                annotation=annotation, title=title, url=url)
                        ...
                        for contributor in l.contributors:
                            ...
                            user = User.objects.filter(first_name=contributor.get(
                                'first_name', ''), last_name=contributor.get('last_name', ''))
                            if user.count() >= 1:
                                user = user.last()
                            else:
                                LOG.warning(
                                    f'No user exists with name `{contributor.get("first_name", "")} {contributor.get("last_name", "")}`.')
                            ...
                            LOG.created(created, 'Contributor',
                                        c.full_name, c.id, warning_color='green')
                            ...
                            if is_author:
                                ...
                                LOG.error(
                                    f'Cannot find a role for contributor {c.full_name}.', kill=True)

                            if not is_current or is_past:
                                LOG.warning(
                                    f'Cannot find whether contributor {c.full_name} is current/past. Will automatically set to past.')
                            ...
                            LOG.created(
                                created, 'Collaboration', f'between {obj.contributor.full_name} and {obj.frontmatter.workshop}', obj.id, warning_color='green')

                    else:
                        LOG.error(
                            f'Have no way of processing {model} for app `frontmatter`. The `setup` script must be adjusted accordingly.', kill=False)
                ...
                for model in l.praxis_models:
                    ...
                    if model == Praxis:
                        ...
                        LOG.created(created, 'Theory-to-practice for',
                                    praxis.workshop, praxis.id)

                    elif model == Tutorial:
                        for annotation in l.tutorials:
                            label, url = process_links(annotation, 'tutorial')
                            obj, created = model.objects.get_or_create(
                                annotation=annotation, label=label, url=url)
                        ...
                        for label in l.discussion_questions:
                            obj, created = model.objects.get_or_create(
                                praxis=praxis, label=label, order=order)
                            LOG.created(
                                created, 'Discussion Question', obj.label, obj.id)
                            order += 1
                        ...
                        for label in l.next_steps:
                            obj, created = model.objects.get_or_create(
                                praxis=praxis, label=label, order=order)
                            LOG.created(created, 'Next Step',
                                        obj.label, obj.id)
                            order += 1

                    else:
                        LOG.error(
                            f'Have no way of processing {model} for app `praxis`. The `setup` script must be adjusted accordingly.', kill=False)
...
def _test_for_branch(d=''):
    ...
    if not branch:
        while not branch:
            branch = get_or_default(
                f'What is the branch name you want to import?', '')
